                        ...
                        for (int j = 0; j< numofprevneurons+1; j++)
                        {
                            ...
                            catch (IndexOutOfRangeException ex)
                            {
                                ...
                                MessageBox.Show($"Индекс выходит за пределы массива!\n" +
                                                $"i: {i}, j: {j}, memory_elemnt.Length: {memory_elemnt.Length}\n" +
                                                $"Ошибка: {ex.Message}",
                                                "Ошибка",
                                                MessageBoxButtons.OK,
                                                MessageBoxIcon.Error);
                                throw; // Выбрасываем ошибку для остановки выполнения, если нужно
                            }
                            catch (FormatException ex)
                            {
                                // Выводим сообщение об ошибке пользователю
                                MessageBox.Show($"Ошибка преобразования строки в число!\n" +
                                                $"i: {i}, j: {j}, значение: {memory_elemnt[j]}\n" +
                                                $"Ошибка: {ex.Message}",
                                                "Ошибка",
                                                MessageBoxButtons.OK,
                                                MessageBoxIcon.Error);
                            ...
                            {
                                // Ловим другие исключения и отображаем их пользователю
